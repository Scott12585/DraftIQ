...
function renderBestAvailable(){
 ...
 list.innerHTML=ranked.length?ranked.map((x,i)=>`<div class="player-line"><span>${i+1}</span>${typeof avatar==='function'?avatar(x.p[0]):''}<div class="name"><b>${E(x.p[0])}</b><small>${typeof nflLine==='function'?nflLine(x.p[0],x.p[1]):x.p[1]}</small></div><div class="value">${x.score.toFixed(1)}</div></div>`).join(''):'<div class="m">No available players at this position.</div>';
}
