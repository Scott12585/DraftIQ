...
function renderCombinedBest(panel){
 ...
 const future=typeof nextMine==='function'?nextMine():null;
 const ranked=overall.map((x,i)=>({x,rank:i+1})).filter(r=>positionFilter==='ALL'||r.x.p[1]===positionFilter).slice(0,12);
 ...
 let html='<div class="cc-rank-head"><span>RK</span><span>PLAYER</span><span>IQ</span><span>AT NEXT</span></div>';
 ranked.forEach(({x,rank})=>{
   ...
   html+=`<div class="cc-rank-row ${targeted?'cc-targeted-row':''}"><div class="cc-rank-num">${rank}</div>${avatar(x.p[0])}<div class="cc-rank-player"><b>${E(x.p[0])}${targetBadge(x.p[0])}</b><small>${metaLine(x.p[0],x.p[1])}</small></div><div class="cc-rank-score">${x.score.toFixed(1)}</div><div class="cc-rank-surv">${Math.round(x.surv)}%</div></div>`;
 });
 ...
 panel.querySelector('.player-list').innerHTML=html;
}
function redesign(){
