...
function assignLineup(){
  ...
  const used=new Set();
  function takePos(pos){const x=pool.map((p,i)=>({p,i})).find(x=>!used.has(x.i)&&x.p.pos===pos);if(!x)return null;used.add(x.i);return x.p;}
  ...
  slots.push({label:'DEF',player:takePos('DEF')});
  const bench=pool.map((p,i)=>({p,i})).filter(x=>!used.has(x.i)).map(x=>x.p);
  while(bench.length<6)bench.push(null);
  return {slots,bench:bench.slice(0,6)};
}
...
rosterView=function(){const {slots,bench}=assignLineup();const roster=myRoster();const starterCount=slots.filter(x=>x.player).length;return `<div class="mt-page lineup-page"><div class="mt-hero"><div><div class="mt-kicker">RED ZONE FRANCHISE HQ</div><h2>Rice Rice Baby</h2><p>Manager: Scott • Live starting lineup and bench assignment.</p></div><div class="mt-status"><strong>${starterCount} / 10</strong><span>STARTERS FILLED</span></div></div><div class="lineup-layout"><section class="mt-panel"><div class="mt-panel-head"><h3>Starting Lineup</h3><span class="m">DraftIQ fills starter spots before bench</span></div><div class="lineup-list">${slots.map(x=>row(x.label,x.player,false)).join('')}</div></section><section class="mt-panel"><div class="mt-panel-head"><h3>Bench</h3><span class="m">${Math.max(0,roster.length-starterCount)} players</span></div><div class="lineup-list">${bench.map((p,i)=>row(`BN ${i+1}`,p,true)).join('')}</div></section></div></div>`;};
