// DraftIQ v0.9.32: Superflex-aware QB demand with a patient QB2 strategy.
// Other teams still value QB2, but Scott's recommendation engine will prioritize RB/WR value early.
(function(){
window.teamNeedMultiplier=function(team,pos){
  const r=teamRoster(team),n=r.filter(x=>x.pos===pos).length;
  if(pos==='QB') return n===0?1.45:n===1?1.05:n===2?.55:.25;
  if(pos==='RB') return n<2?1.30:n<4?1.05:.80;
  if(pos==='WR') return n<3?1.35:n<5?1.08:.82;
  if(pos==='TE') return n===0?1.15:n===1?.65:.35;
  return 1;
};
window.posNeed=function(pos){
  const n=myRoster().filter(x=>x.pos===pos).length;
  if(pos==='QB'){
    // Scott already keeps Josh Allen. De-emphasize QB2 in rounds 1-2,
    // begin increasing interest in round 3, and make it a stronger need in round 4+.
    if(n===0) return 82;
    if(n===1){
      const r=round(s.pick);
      if(r<=2) return 18;
      if(r===3) return 36;
      if(r===4) return 55;
      return 68;
    }
    return n===2?22:10;
  }
  if(pos==='RB') return Math.max(18,90-n*23);
  if(pos==='WR') return Math.max(20,95-n*20);
  if(pos==='TE') return n?25:58;
  return 20;
};
})();